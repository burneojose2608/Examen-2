...
val productosValidos = inventario.filter { p =>
  p.precios.length >= minPrecios &&
    p.precios.max > valorBase
}


val listaProductoPromedio: List[ProductoPromedio] = productosValidos.map { p =>
  var suma = 0.0
  for (precio <- p.precios) {
    suma += precio
  }
  val promedio = suma / p.precios.length
  ProductoPromedio(p, promedio)
}

var mejor: ProductoPromedio = listaProductoPromedio.head

for (pp <- listaProductoPromedio.tail) {
  if (pp.promedio > mejor.promedio) {
    mejor = pp
  }
}

println(s"Producto con mayor promedio: ${mejor.producto.nombre}")
println(s"Promedio: ${mejor.promedio}")